...
public class Game{
  ...
  public void addPlayer(Player p)throws IllegalArgumentException{
    if(p == null)
      throw new IllegalArgumentException("Le joueur ne peut pas être null, taper un bon joueur");
  ...
  public void play()throws InvalidNumberPlayerException{
    ...
    while(Game.hasWon(currentPlayer)){
      ...
      if(currentCell.canLeave()){
        ...
        System.out.println(currentPlayer.toString()+" est dans  "+currentCell.toString()+", "+currentPlayer.toString()+" en peut pas jouer.");
      }
    }
    System.out.println(currentPlayer.toString()+" a gagné.");
  }
